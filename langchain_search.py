...
connection_string = CONNECTION_STRING.format(user, passwd)
...
# https://github.com/langchain-ai/langchain/blob/master/libs/community/langchain_community/embeddings/openai.py#L496
# fixed by changing `input=tokens[i : i + _chunk_size]` to `input=texts[0]`
# if an ssl certificate error is raised, Install Certificates.command
...
# Tools part 2
# tools for retrieving specific documents based on popular searches criteria
...
# Create the agent
# The create_conversational_retrieval_agent is a built-in agent that includes conversational history
# as well uses the OpenAIFunctionsAgent as its underlying implementation.
...
# an agent leverages the LLM to assess the incoming request with the current context to decide what steps or actions need to be executed and in what order
# LangChain agents can leverage tools
